...
function submitFeedback() {
  ...
  if (!name || !email || rating === "0" || !feedback) {
    document.getElementById('feedback-response').innerHTML =
      '<p class="text-danger">Please fill out all fields, including rating.</p>';
    return;
  }
  ...
  fetch('submit_feedback.php', {
    ...
    .catch(error => {
      document.getElementById('feedback-response').innerHTML =
        '<p class="text-danger">Failed to submit feedback. Try again later.</p>';
    });
}
      ...
      data.forEach(item => {
        feedbackList.innerHTML += `
          <div class="feedback-item">
            <p><strong>${item.name}</strong> (${item.rating} stars)</p>
            <p>${item.message}</p>
            <hr>
          </div>`;
      });
